...
namespace xgboost
{
    ...
    public:
        ...
        inline const std::vector<size_t> &LabelAbsSort() const
        {
            ...
            XGBOOST_PARALLEL_STABLE_SORT(label_order_cache_.begin(), label_order_cache_.end(), [&l](size_t i1, size_t i2) { return std::abs(l[i1]) < std::abs(l[i2]); });
    ...
    struct BatchParam
    {
        ...
        // Hist
        BatchParam(bst_bin_t max_bin, double sparse_thresh) : max_bin{max_bin}, sparse_thresh{sparse_thresh}
        {}
         ...
         */
        BatchParam(bst_bin_t max_bin, common::Span<float> hessian, bool regenerate) : max_bin{max_bin}, hess{hessian}, regen{regenerate}
        {}
    ...
    public:
        using iterator_category = std::forward_iterator_tag;  // NOLINT
        explicit BatchIterator(BatchIteratorImpl<T> *impl)
        {
            impl_.reset(impl);
        }

        explicit BatchIterator(std::shared_ptr<BatchIteratorImpl<T>> impl)
        {
            impl_ = impl;
        }
    ...
    public:
        ...
        BatchIterator<T> end()
        {
            return BatchIterator<T>(nullptr);
        }  // NOLINT
